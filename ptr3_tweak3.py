#!/usr/bin/env python3
"""
PTR3 - Tweak 3: Add Gold Cross-Asset Momentum Filter

When BOTH gold (GLD) AND treasury momentum (IEF) are positive: allocate to gold + stocks
Otherwise: allocate to treasuries (IEF) as per original strategy
"""
...
def calculate_signals(month_end_data, daily_data):
    ...
    for i, current_date in enumerate(month_dates):
        ...
        spy = data['SPY'].dropna()
        dbc = data['DBC'].dropna()
        gld = data['GLD'].dropna() if 'GLD' in data.columns else None
        ...
        # TWEAK 3: Gold cross-asset momentum filter
        # If gold momentum positive AND IEF (treasury) momentum positive: +1
        # Otherwise: -1 (stick with treasuries)
        if gld is not None and len(gld) > 252 and len(ief) > 252:
            gld_12m = (gld.iloc[-1] - gld.iloc[-252]) / gld.iloc[-252]
            ief_12m = (ief.iloc[-1] - ief.iloc[-252]) / ief.iloc[-252]
            
            # Both positive = use gold signal (+1)
            if gld_12m > 0 and ief_12m > 0:
                signals['gold_filter'].append(1)
            else:
                signals['gold_filter'].append(-1)
        else:
            signals['gold_filter'].append(0)
...
def create_signals_dataframe(signals):
    ...
    df['avg_signal'] = df[['yield_spread', 'bond_trend', 'equity_returns', 'commodity_returns', 'gold_filter']].mean(axis=1)
...
if __name__ == '__main__':
    print("Tweak 3: Gold Cross-Asset Momentum Filter")
    print("=" * 50)
    
    price_data = get_etf_data([IEF, BIL, SPY, DBC, GLD], START_DATE, END_DATE)
    ...
    signals_df.to_excel('ptr3_tweak3_position.xlsx')
    print("Saved ptr3_tweak3_position.xlsx")
    ...
    plt.title('PTR3 Tweak 3: Gold Cross-Asset Filter (0-100%)')
    plt.xlabel('Date')
    plt.ylabel('Signal (%)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('ptr3_tweak3_position.png', dpi=150)
    plt.close()
    print("Saved ptr3_tweak3_position.png")
    
    equity_df = calculate_portfolio_equity(signals_df, price_data)
    equity_df.to_excel('ptr3_tweak3_equity.xlsx')
    print("Saved ptr3_tweak3_equity.xlsx")
    
    plt.figure(figsize=(12, 6))
    plt.plot(equity_df.index, equity_df['cumulative_equity'], 'g-', linewidth=1.5)
    plt.title('PTR3 Tweak 3: Cumulative Portfolio Equity')
    plt.xlabel('Date')
    plt.ylabel('Cumulative Equity')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('ptr3_tweak3_equity.png', dpi=150)
    plt.close()
    print("Saved ptr3_tweak3_equity.png")
    
    ann_return, max_dd = calculate_performance(equity_df)
    print(f"\n{'='*50}")
    print("PTR3 Tweak 3 Performance")
    print(f"{'='*50}")
