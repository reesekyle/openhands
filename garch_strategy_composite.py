"""
COMPOSITE Strategy: Combining Best Modifications
================================================
Combines:
- Mod 3: Two-filter signal (rate change + volatility) - BEST performer
- Mod 4: Adaptive thresholds based on market regime

This composite attempts to further improve on mod3's 8.71% return.
...
def download_data(start_date='2000-01-01', end_date=None):
    ...
    # Rate change (level diff more stable)
    ...
    # Market volatility for adaptive thresholds
...
def generate_signals(data, base_vol_threshold=1.5, base_change_threshold=-1.0, regime_sensitivity=0.3):
    """
    COMPOSITE: Two-filter signal with adaptive thresholds.
    
    Combines:
    - Mod 3's two-filter approach (rate change + volatility)
    - Mod 4's adaptive threshold based on market regime
    
    Exit when EITHER:
    1. Interest rate change is significantly negative (rates rising)
    2. Interest rate volatility > adaptive threshold
    """
    df = data.copy()
    ...
    # In high vol markets, lower threshold (more protective)
    adaptive_vol_threshold = base_vol_threshold * (1.5 - regime_sensitivity * market_vol_percentile)
    adaptive_change_threshold = base_change_threshold * (1.5 - regime_sensitivity * market_vol_percentile)
    
    # Signal: 1 = LONG, 0 = FLAT
    df['Signal'] = np.where(
        (df['Vol_zscore'] > adaptive_vol_threshold) | (df['Change_zscore'] < adaptive_change_threshold),
        0,  # FLAT
        1   # LONG
    )
...
def main():
    print("=" * 60)
    print("COMPOSITE: Two-Filter + Adaptive Thresholds")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PERFORMANCE METRICS - COMPOSITE")
    print("=" * 60)
    for key, value in metrics.items():
        print(f"{key}: {value}")
    
    # Plot
    plot_performance(df, save_path='/workspace/project/openhands/garch_performance_composite.png')
