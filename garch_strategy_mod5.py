"""
MOD 5: Long/Short Variant
==========================
Based on paper's finding that both rate changes and volatility negatively impact returns.

Key difference from original:
- LONG when interest rates falling/low volatility (favorable conditions)
- SHORT when interest rates rising/high volatility (unfavorable conditions)
- More aggressive - captures both directions
...
def generate_signals(data, vol_threshold=None, change_threshold=None):
    ...
    - LONG (+1) when rates falling AND low volatility (favorable)
    - SHORT (-1) when rates rising OR high volatility (unfavorable)
    """
    df = data.copy()
    ...
    # Signal: 1 = LONG, -1 = SHORT (flat no longer used)
    # Long when: low vol AND rates not rising
    # Short when: high vol OR rates rising
    df['Signal'] = np.where(
...
def main():
    print("=" * 60)
    print("MOD 5: Long/Short Variant")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PERFORMANCE METRICS - MOD 5")
    print("=" * 60)
    for key, value in metrics.items():
        print(f"{key}: {value}")
    
    # Plot
    plot_performance(df, save_path='/workspace/project/openhands/garch_performance_mod5.png')
