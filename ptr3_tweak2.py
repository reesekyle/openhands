#!/usr/bin/env python3
"""
PTR3 - Tweak 2: Use longer duration when yields high, shorter when yields low

When 10-year yields are HIGH (>3%): use longer duration (TLT)
When 10-year yields are LOW (<3%): use shorter duration (IEF)
"""
...
def calculate_signals(month_end_data, daily_data):
    """Calculate signals with Tweak 2: Duration rotation based on yield level"""
    ...
    for i, current_date in enumerate(month_dates):
        ...
        # TWEAK 2: Duration rotation
        # If yields HIGH (>3%): use longer duration (TLT) = +1
        # If yields LOW (<3%): use shorter duration (IEF) = -1
        # Use IEF price as proxy - lower price = higher yields
        if len(ief) > 21:  # Need at least 1 month
            ief_price = ief.iloc[-1]
            # Approximate: if price < 95, yields are high (>3%)
            # This is a rough approximation
            yield_threshold = 3.0
            
            # Estimate yield from price (rough inverse relationship)
            estimated_yield = 100 / ief_price if ief_price > 0 else 2
            
            if estimated_yield > yield_threshold:
                # Yields high -> use TLT (+1)
                signals['duration_signal'].append(1)
            else:
                # Yields low -> use IEF (-1)
                signals['duration_signal'].append(-1)
        else:
            signals['duration_signal'].append(0)
...
def create_signals_dataframe(signals):
    ...
    df['avg_signal'] = df[['yield_spread', 'bond_trend', 'equity_returns', 'commodity_returns', 'duration_signal']].mean(axis=1)
...
def calculate_portfolio_equity(signals_df, price_data):
    ...
    for i in range(len(signals_df) - 1):
        ...
        # Use IEF (shorter duration) as base, TLT for longer
        if 'IEF' in price_data.columns and 'TLT' in price_data.columns and 'BIL' in price_data.columns:
            ief_prices = price_data['IEF'].dropna()
            tlt_prices = price_data['TLT'].dropna()
...
if __name__ == '__main__':
    print("Tweak 2: Duration Rotation Based on Yield Level")
    print("=" * 50)
    
    price_data = get_etf_data([IEF, TLT, BIL, SPY, DBC], START_DATE, END_DATE)
    ...
    signals_df.to_excel('ptr3_tweak2_position.xlsx')
    print("Saved ptr3_tweak2_position.xlsx")
    ...
    plt.title('PTR3 Tweak 2: Duration Rotation (0-100%)')
    plt.xlabel('Date')
    plt.ylabel('Signal (%)')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('ptr3_tweak2_position.png', dpi=150)
    plt.close()
    print("Saved ptr3_tweak2_position.png")
    
    equity_df = calculate_portfolio_equity(signals_df, price_data)
    equity_df.to_excel('ptr3_tweak2_equity.xlsx')
    print("Saved ptr3_tweak2_equity.xlsx")
    
    plt.figure(figsize=(12, 6))
    plt.plot(equity_df.index, equity_df['cumulative_equity'], 'g-', linewidth=1.5)
    plt.title('PTR3 Tweak 2: Cumulative Portfolio Equity')
    plt.xlabel('Date')
    plt.ylabel('Cumulative Equity')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('ptr3_tweak2_equity.png', dpi=150)
    plt.close()
    print("Saved ptr3_tweak2_equity.png")
    
    ann_return, max_dd = calculate_performance(equity_df)
    print(f"\n{'='*50}")
    print("PTR3 Tweak 2 Performance")
    print(f"{'='*50}")
